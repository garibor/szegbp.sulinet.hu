...
<?php 
	...
	$pageName = $_GET['page'];
	$news = $_GET['news'];
	$query = "SELECT id, title, type, text FROM szeg_page WHERE name='".$pageName."'";		
		...
		<div class="content-container">
			...
			<?php
				...
				if($pageName == 'aktualitasok'){
					$newsQuery = "SELECT id, title, text, date, time, author_id, status, type FROM szeg_news WHERE status != 0 ORDER BY date DESC";
					$newsResult = mysqli_query($connection, $newsQuery);
					...
					if($row['type'] == 2){
						?>
						<script>	
							window.open('doc/<?php echo $row['text'] ?>');
						</script>
						Dokumentumot innen tudja letölteni!
						<a href="new/<?php echo $row['text'];?>" download>Jee</a>
						<?php			
					}else{
