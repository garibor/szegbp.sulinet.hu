		...
		<div class="content">	
			<h2><?php 
				if(!$select_year){
					...
					echo("Naptár - ".$select_year."-".$select_month."-".$select_day);
				}
				?></h2>	
			<div id="caleandar">
			...
			<div class="right-box" style="margin-left: 630px;">
				...
				<form  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
					...
					<p style="margin-top: 15px;">
						<label for="myCheck" style=" font-size: 15px;">Kiemelt esemény:</label> 
					...
					<p id="text" <?php echo $show; ?>>
						<label class="col-form-label" for="inputDefault">Esemény kép URL</label>
						<input type="text" name="event-img" class="form-control" style="width: 300px;" value="<?php echo $row['img']; ?>">
					</p>
					
					<p style="margin-top: 30px;">
						<button type="submit" name="update-event" class="btn btn-outline-success save-user">Frissítés</button>
					</p>
				</form>
			</div>
			<?php }} ?>
			...
			<?php }else if(!$event_id){
				...
				<div class="right-box" style="margin-left: 630px;">
					...
					if(mysqli_num_rows($eventResult)== 0){
							echo "Jelenleg nincs esemény felvéve";
							?>
			...
			<?php
				...
				if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update-event'])) {
    				...
    				if ($connection->query($updateEvent) === TRUE) {
    					...
    					?>
						<div class="alert alert-dismissible alert-danger">
						  <strong>Hiba!</strong> Esemény létrehozása során hiba lépett fel. <?php echo ($updateEvent."  ".$connection->error); ?>
						</div>
						<?php	
		            } 
				}
				...
				if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete-event'])) {
    				...
    				if ($connection->query($deletEvent) === TRUE) {
                		?>
						<div class="alert alert-dismissible alert-success">
							<strong>Sikeresen</strong> törölve lett ez esemény!
						</div>
						<script>
    					...
    					?>
						<div class="alert alert-dismissible alert-danger">
						  <strong>Hiba!</strong> Esemény törlése során hiba lépett fel. <?php echo ($deletEvent."  ".$connection->error); ?>
						</div>
						<?php		
		            } 	
				}
